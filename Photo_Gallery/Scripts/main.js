;(function () {
    ...
    $("#allphotos").click(function () {
        console.log("+");
        for (var ph = 2; ph < photosessions.length; ph++) {
            photosessions.eq(ph).show();
    ...
    $('#btn_connectUs').click(function () {
        var popup_id = $('#' + $(this).attr("rel"));
        popup_id.show();
        //$('.overlay_popup').show();
        $('.popup').removeClass("hideSafari");
        $('.popup').show();
        $('.main').fadeIn();
        $('.popup').addClass('opend-popup');
...
// закрыть на крестик
    $('.js-login-close-campaign, .close').click(function () { 
        ...
        $('.popup').css("display", "");
        $('.login_overlay_popup, .login_popup, .popup, .overlay_popup').removeClass("opend-popup");
        $('.login_overlay_popup, .login_popup, .popup, .overlay_popup').hide();
        $('.main, .slideInDown').removeClass('hideMain');
        $('#fh5co-header').removeClass('hideMain');
        $('#responce').text("Отправте нам соощение!");
